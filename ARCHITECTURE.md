...
- **Focus Lease (multi-Client policy).** A tmux pane has one real size, so geometry concurrency is resolved by lease, not by merging: at most one Client holds a Pane's Focus Lease, and the lease follows *driving* — a Client's first keystroke or `ResizeRequest` on a Pane steals it implicitly from any current holder (it's the same developer — steal, never block; there is no explicit acquire message, ADR-0013). Selecting or viewing acquires nothing, so browsing the Herd is wire-silent. What the lease makes exclusive is **geometry**: the Pane's canonical size follows the holder, and the pre-drive size record — created when an epoch's first `ResizeRequest` displaces the Pane's size — travels with the lease across steals, never re-recorded at an intermediate size. Only the holder consumes it: `RestoreSize` from the holder, or the holder stream's death, returns the Pane to its true pre-drive size and ends the epoch; a robbed stream's `RestoreSize` is a no-op and its death restores nothing. Input, by contrast, is stolen-not-blocked: a non-holder's keystroke makes its sender the holder and is delivered, never dropped — silently eating the developer's own keystrokes would be worse than interleaving them, and concurrent typing from two Clients interleaves exactly as two attached tmux clients would; the lease makes that legible, not impossible. Non-holders render the canonical grid read-only (scrolling their viewport if smaller); lease changes broadcast on the Telemetry Stream so every Client can show who's driving.
...
| Control Input | Client → Server | `KeyBytes{pane, bytes}`, `Paste{pane, bytes}`, `Mouse{pane, x,y,btn,mods}`, `ResizeRequest`, `RestoreSize` (the Server records the pre-drive size displaced by an epoch's first resize of a Pane; the record travels with the Focus Lease across steals, and only the holder consumes it — by request or when the holder's stream dies) — no explicit lease acquire: a stream's first input/resize on a Pane steals its Focus Lease (ADR-0013) |
