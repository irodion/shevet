...
- **Dashboard surfaces: Tiles, Overview, Focus** (CONTEXT.md senses). **Tiles** is the default: the whole viewport divided among the Herd by weighted binary splits — a Pane joining splits a tile, a leaving one returns its space. A tile is a cursor-following viewport onto its Pane at the Pane's real size; the layout never resizes Panes (ADR-0014). Organization belongs to the developer, by verbs: spatial (directional) navigation, directional swap, grow/shrink, equalize/reset, dismiss-Exited. Default order is arrival order, rearranged only by verbs — Status decorates a tile but never moves it (with passthrough input, a tile that moved under the developer's fingers would misroute keystrokes). Order and weights are session-scoped; durable layouts are deliberately out of scope (Pane identity is ephemeral — a persisted tree would mis-slot a churned Herd). Tiles never shrink below a readable minimum: Panes past capacity collapse into one final "+N more" tile that rolls up their Status; Overview shows the whole Herd. **Overview** is a transient orientation surface (the OS window-overview analogue): uniform cards behind a dedicated verb — picking a Pane jumps straight to Focus on it, dismissing returns to Tiles. **Focus** devotes the viewport to one Pane, resized to the viewport minus a persistent one-row Shevet bar carrying the focused Pane's identity (title, PaneRef) and live Status; unfocus restores the pre-focus size.
- **Input model: passthrough-by-default, Leader-reserved** (ADR-0013). In Tiles and Focus, every unmodified keystroke is re-encoded to exact bytes and forwarded to the selected/focused Pane's Agent; every dashboard verb — navigate, swap, resize, Overview, Focus, quit — lives behind the single configurable Leader (default `Ctrl-\`). Enter is an Agent key (answering a prompt needs it), so entering Focus is a Leader verb, never Enter. Overview forwards nothing; its bare keys are dashboard verbs.
...
- **Focus Lease (multi-Client policy).** A tmux pane has one real size, so geometry concurrency is resolved by lease, not by merging: at most one Client holds a Pane's Focus Lease, and the lease follows *geometry driving* — a Client's `ResizeRequest` on a Pane steals it implicitly from any current holder (it's the same developer — steal, never block; there is no explicit acquire message, ADR-0013). Keystrokes never steal it: input is always delivered — concurrent typing from two Clients interleaves exactly as two attached tmux clients would, and silently eating the developer's own keystrokes would be worse — and since only Focus resizes a Pane (ADR-0014), typing into a tile can never strand another Client's Focus geometry: the focused Client keeps the lease and its unfocus restore stays honored. Selecting, viewing, and typing are lease-silent; browsing the Herd is wire-silent. What the lease makes exclusive is **geometry**: the Pane's canonical size follows the holder, and the pre-drive size record — created when an epoch's first `ResizeRequest` displaces the Pane's size — travels with the lease across steals, never re-recorded at an intermediate size. Only the holder consumes it: `RestoreSize` from the holder, or the holder stream's death, returns the Pane to its true pre-drive size and ends the epoch; a robbed stream's `RestoreSize` is a no-op and its death restores nothing. Every Client renders the canonical grid at the holder's geometry (scrolling its viewport if smaller); lease changes broadcast on the Telemetry Stream so every Client can show who holds a Pane's geometry.
...
| Control Input | Client → Server | `KeyBytes{pane, bytes}`, `Paste{pane, bytes}`, `Mouse{pane, x,y,btn,mods}`, `ResizeRequest`, `RestoreSize` (the Server records the pre-drive size displaced by an epoch's first resize of a Pane; the record travels with the Focus Lease across steals, and only the holder consumes it — by request or when the holder's stream dies) — no explicit lease acquire: a stream's `ResizeRequest` on a Pane steals its Focus Lease; keystrokes never do (ADR-0013) |
