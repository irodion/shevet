...
The spool guarantee (ADR-0005) is unchanged and lives where it always conceptually did — in the caller: when the CLI cannot reach the socket, it appends the event to `~/.shevet/signals.spool` per the spool protocol, and the Server drains on startup. "Cannot reach" means transport-level failure only — socket absent, connection refused, or the call's deadline exceeded (hook calls carry a short per-call deadline, low single-digit seconds, so a wedged Server never hangs the Agent's hook). A reply from the Server — including an error reply, e.g. validation — is a delivered Signal and is surfaced, never spooled: replaying a rejected request later would just replay the rejection.
