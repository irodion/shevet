...
- **Exited-until-Dismissed:** Spawn sets `remain-on-exit on` on each window it creates — it is a window/pane option with no per-session scope, so it is applied at the same moment as the provenance option, never assumed from session state. A Spawned Agent's death leaves a dead pane in tmux with its final screen intact — `capture-pane` still works, Status `Exited` derives from `#{pane_dead}` — until Dismiss, which is `kill-pane`. The retained screen survives Server restarts because it never left tmux.
